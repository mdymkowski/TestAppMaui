...
public sealed class EfCoreLocalTaskStore : ILocalTaskStore
{
    ...
    public EfCoreLocalTaskStore(IDbContextFactory<ApplicationDbContext> contextFactory)
    {
    ...
    public async Task ReplaceTasksAsync(IEnumerable<TaskDto> tasks, CancellationToken cancellationToken = default)
    {
        ...
        {
            context.Tasks.Add(TaskItem.FromExisting(task.Id, task.Title, task.Description, task.DueDate, task.IsCompleted));
        }
